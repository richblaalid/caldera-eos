  ...
  try {
    ...
    for (const nudge of filtered) {
      ...
      if (nudge.escalation === 3) {
        await createEscalationIssue(organizationId, nudge)
...
/**
 * Create an L10 escalation Issue when a nudge reaches level 3.
 */
async function createEscalationIssue(organizationId: string, nudge: Nudge): Promise<void> {
  // Check for duplicate
  const { data: existing } = await supabaseAdmin
    .from('issues')
    .select('id')
    .eq('organization_id', organizationId)
    .ilike('title', `%${nudge.itemTitle}%`)
    .eq('status', 'open')
    .limit(1)
  ...
  await supabaseAdmin.from('issues').insert({
    ...
    title: `[Escalated] ${nudge.title}`,
    description: `[Auto-generated by Ember Nudge Engine]\n\nOwner: ${nudge.targetPartnerName}\n\n${descriptionByType[nudge.type]}`,
    status: 'open',
    priority: 'high',
    owner_id: nudge.targetPartnerId,
    source: 'insight',
  })
}
